...
/**
 * Controlador de la colonia de hormigas que usa la distancia para la feromona
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class ColHFerDist extends Controlador {

    /**
     * Hormigas
     */
    private HormigaFerDist[] hormigas;
     ...
     */
    public ColHFerDist(Juego juego) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaFerDist[87];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaFerDist(grafo, padre.getActual(), new Posicion(5, -1));
            } else {
                hormigas[i] = new HormigaFerDist(grafo, padre.getActual(), new Posicion(5, 17));
            }
        }
    }
     ...
     */
    public ColHFerDist(Juego juego, int nHormigas) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaFerDist[nHormigas];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaFerDist(grafo, padre.getActual(), new Posicion(5, -1));
    ...
    public void mover(Juego juego) throws CloneNotSupportedException {
        ...
        while (!fin && !Thread.interrupted() && !finTurno) {
            ...
            // que 0.1 no mueve
            if (!primerMov && enlaces[enlace].getFeromona() < 0.1) {
                return;
            }
