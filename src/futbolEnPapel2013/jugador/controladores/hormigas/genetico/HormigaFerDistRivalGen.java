...
/**
 * Clase que representa una hormiga que usa la distancia y el mínimo de la
 * distancia por el coste del rival para dejar feromona
 * cuyos parámetros han sido optimizados mediante un algoritmo genético
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class HormigaFerDistRivalGen {
    ...
    /**
     * Lista de los enlaces que ha marcado esta hormiga
     */
    private ArrayList<Enlace> marcados;
    ...
    /**
     * El parámetro alfa de la actualización de las tablas de decisión
     */
    static double alfa;

    /**
     * El parámetro beta de la actualización de las tablas de decisión
     */
    static double beta;

    /**
     * El parámetro ro de la disipación de feromona
     */
    static double ro;

    /**
     * Parámetro usado para guiar un poco a las hormigas restando probabilidad
     * a los nodos más lejanos de la portería
     */
    static double restaPorDistancia;

    /**
     * Peso del coste para dejar feromona
     */
    static double pesoCoste;

    /**
     * Peso para la distancia
     */
    static double pesoDistancia;

    /**
     * Peso para la distancia · coste del rival
     */
    static double pesoRival;
     ...
     */
    public HormigaFerDistRivalGen(Grafo gr, Posicion base, Posicion porteria) {
        grafo = gr;
        hormiguero = base;
        meta = porteria;
        camino = new ArrayList<Posicion>();
        marcados = new ArrayList<Enlace>();
...
//                                    g.getGrafo().getNodos()[i][j].getEnlaces()[k].getCoste(1))
    ...
    public boolean mover() {
            ...
            if (camino.get(camino.size() - 1).getY() < 17  && camino.get(camino.size() - 1).getY() > -1) {
                ...
                if (camino.get(camino.size() - 1).getY() == meta.getY()) {
                    ...
                    if (camino.size() > 1) {
                        ...
                        marcados.remove(marcados.size() - 1);
                        actual = camino.get(camino.size() - 1);
                        if (camino.size() > 1) {
                            anterior = camino.get(camino.size() - 2);
                        } else {
         ...
         * marcados o pertenecen a marcados o los nodos conectados son tabú
...
//                    costeTotal -= Math.min(aBorrar.getCoste(0), aBorrar.getCoste(1));
        ...
        // Se hace lo mismo si el enlace pertenece a marcados.
        // No es necesario comprobar si el enlace está marcado en el grafo
        // porque ya se pone a 0 en la tabla de decisión del nodo
        for (int i = 0; i < actNodo.getEnlaces().length; i++) {
            Posicion aux = Grafo.obtenerPosNodo(actual, i);
            if (tabu.contains(aux)
                    || marcados.contains(actNodo.getEnlaces()[i])) {
                algunoTabu = true;
                tablaProb[i] = 0;
            }
        }
        ...
        // Se resta a la probabilidad de cada nodo restaPorDistancia * distancia
        // a la meta restando al final el mínimo si es menor que 0 y normalizando
        for (int i = 0; i < tablaProb.length; i++) {
            if (tablaProb[i] != 0) {
                tablaProb[i] -= restaPorDistancia * distancia(Grafo.obtenerPosNodo(actual, i));
            }
        }
        ...
        // La feromona que se deja está en relación al coste del enlace y de la
        // distancia a la meta rival y del mínimo
        // de la distancia a la portería propia por el coste (para aumentar la
        // distancia en turnos del rival a la portería propia)
        // (pesoCoste/coste + pesoDistancia/distancia + pesoRival*min)
        double suma = 0;
        double coste = 1D / actNodo.getEnlaces()[mejor].getCoste
                (grafo.obtenerIndiceCoste(actual, Grafo.obtenerPosNodo(actual, mejor)));
        coste *= pesoCoste;
        double distancia = distancia(Grafo.obtenerPosNodo(actual, mejor));
        distancia = pesoDistancia / distancia;

        // Se calcula el mínimo
        Posicion sig = new Posicion(actual.getX() - 1, actual.getY());
        double min = 2E09;
        for (int i = 0; i < actNodo.getEnlaces().length; i++) {
            // Se calcula cuál es la posición del nodo siguiente
            switch (i) {
                case 1:
                    sig = new Posicion(actual.getX() + 1, actual.getY());
                    break;
                case 2:
                    sig = new Posicion(actual.getX(), actual.getY() - 1);
                    break;
                case 3:
                    sig = new Posicion(actual.getX(), actual.getY() + 1);
                    break;
                case 4:
                    sig = new Posicion(actual.getX() - 1, actual.getY() - 1);
                    break;
                case 5:
                    sig = new Posicion(actual.getX() + 1, actual.getY() - 1);
                    break;
            ...
            if (actNodo.getEnlaces()[i] != null && !actNodo.getEnlaces()[i].getMarcado()) {
                double este = actNodo.getEnlaces()[i].getCoste
                    (grafo.obtenerIndiceCoste(actual, sig)) * distanciaAPropia(sig);
                if (este < min) {
                    min = este;
                }
            }
        ...
        suma = (coste + distancia + pesoRival * min);

        actNodo.getEnlaces()[mejor].sumaFerNueva(suma);

        camino.add(Grafo.obtenerPosNodo(actual, mejor));
        marcados.add(actNodo.getEnlaces()[mejor]);
...
//        costeTotal += Math.min(actNodo.getEnlaces()[mejor].getCoste(0), actNodo.getEnlaces()[mejor].getCoste(1));
     ...
     */
    public void volver() {
//        double sumaFer = Math.pow(distancia(hormiguero) / costeTotal, 4);
...
//        aux.sumaFerNueva(1D / Math.min(aux.getCoste(0), aux.getCoste(1)));
     ...
     * @return La distancia a la meta
     */
    public double distanciaAPropia(Posicion pos) {
        double resultado = 0;
