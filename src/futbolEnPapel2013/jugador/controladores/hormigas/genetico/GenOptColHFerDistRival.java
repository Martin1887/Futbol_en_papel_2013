 ...
 * la distancia y el mín. de la distancia por el coste del rival para dejar feromona.
 ...
 * pesoRival * 10 - 1 (tiene que tener mayores valores y la precisión decimal no importa tanto)
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class GenOptColHFerDistRival {
     ...
     * @param nivel El nivel del alfa-beta contra el que enfrentarse
    ...
    public static double fEvaluacion(BitSet[] individuo, int elemento, boolean intermedia, BufferedWriter fSalida,
        ...
        int niv1 = Juego.C_HORMIGAS_FER_DIST_RIVAL_GEN_EVAL;
        int niv2 = nivel;
    ...
    public static void main(String[] args) throws Exception {
         ...
         * (8 parámetros)
         */
        BitSet[][] generacion = new BitSet[16][10];
...
//        BufferedReader teclado = new BufferedReader(new InputStreamReader(System.in));
        ...
        String ruta = "evMejorDistRival.txt";
...
//        }
        ...
        for (int i = 0; i < nHilos; i++) {
            ...
            EvaluacionFerDistRivalGen evaluador = new EvaluacionFerDistRivalGen(individuos, evaluaciones,
                    primerElemento, false, lineaT, estadisticas, barrera, 1);
            evaluador.start();
...
//            }
            ...
            tamTrozo = 1;
            primerElemento = 0;
            // Si la generación es mayor que 20 se enfrenta contra el alfa-beta 2,
            // y si es la 20 hay que evaluar todos los individuos (no solo la intermedia),
            // porque si no los de la anterior tendrían la evaluación contra 
            // el 1 en vez de contra el 2
            if (nGeneracion == 300) {
                // El tamaño de la barrera es el número de hilos a ejecutar más este hilo
                barrera = new CyclicBarrier(nHilos + 1);
                tamTrozo = generacion.length / nHilos;
                primerElemento = 0;
                for (int i = 0; i < nHilos; i++) {
                    BitSet[][] individuos = new BitSet[tamTrozo][generacion[0].length];
                    System.arraycopy(generacion, primerElemento, individuos, 0, tamTrozo);
                    EvaluacionFerDistRivalGen evaluador = new EvaluacionFerDistRivalGen(individuos, evaluaciones,
                            primerElemento, false, lineaT, estadisticas, barrera, 2);
                    evaluador.start();
                    primerElemento += tamTrozo;
                }
                barrera.await();
            } else {
                int niv = (nGeneracion < 300) ? 1 : 2;
                for (int i = 0; i < tamano; i++) {
                    ...
                    EvaluacionFerDistRivalGen evaluador = new EvaluacionFerDistRivalGen(individuos, evaluacionesIntermedia,
                            primerElemento, true, lineaT, estadisticasIntermedia, barrera, niv);
                    evaluador.start();
                    primerElemento += tamTrozo;
                }
                barrera.await();
