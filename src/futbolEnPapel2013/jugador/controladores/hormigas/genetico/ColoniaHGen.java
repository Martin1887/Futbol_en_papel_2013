...
/**
 * Controlador de la colonia de hormigas optimizada mediante un algoritmo
 * genético
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class ColoniaHGen extends Controlador {

    /**
     * Hormigas
     */
    private HormigaGen[] hormigas;
     ...
     * genético
     */
    public ColoniaHGen(Juego juego) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaGen[3];
        nCiclos = 50;
        HormigaGen.alfa = 0.81;
        HormigaGen.beta = 1.68;
        HormigaGen.ro = 0.64;
        HormigaGen.restaPorDistancia = 1.31;
        grafo = new Grafo(padre.getTablero());
        evGenetico = false;
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaGen(grafo, padre.getActual(), new Posicion(5, -1));
            } else {
                hormigas[i] = new HormigaGen(grafo, padre.getActual(), new Posicion(5, 17));
            }
        }
    }
     ...
     */
    public ColoniaHGen(Juego juego, int nHorm, int nCic, double alf,
            double bet, double r, double restaPorDist,
            int elem, boolean inter) throws CloneNotSupportedException {
        evGenetico = true;
        padre = juego;
        nHormigas = nHorm;
        nCiclos = nCic;
        ...
        HormigaGen.beta = bet;
        HormigaGen.ro = r;
        HormigaGen.restaPorDistancia = restaPorDist;
        elemento = elem;
        intermedia = inter;
        hormigas = new HormigaGen[nHormigas];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaGen(grafo, padre.getActual(), new Posicion(5, -1));
    ...
    public void mover(Juego juego) throws CloneNotSupportedException {
        ...
        if (evGenetico) {
            if (intermedia) {
                GeneticoOptColH.nTurnosInter[elemento]++;
            } else {
                GeneticoOptColH.nTurnos[elemento]++;
            }
        }
            ...
            if (!paro) {
                ...
                if (bloqueo()) {
                    if (evGenetico) {
                        ...
                        if (intermedia) {
                            GeneticoOptColH.tiempoTotalInter[elemento] = t2 - t1;
                        } else {
                            GeneticoOptColH.tiempoTotal[elemento] = t2 - t1;
                        }
                    }
                    return;
                }
