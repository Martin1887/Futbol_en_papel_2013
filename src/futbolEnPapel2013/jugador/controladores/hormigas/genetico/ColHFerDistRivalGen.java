...
/**
 * Controlador de la colonia de hormigas que usa la distancia y la distancia por
 * el coste del camino mínimo para el rival para dejar feromona
 * optimizada mediante un algoritmo genético
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class ColHFerDistRivalGen extends Controlador {

    /**
     * Hormigas
     */
    private HormigaFerDistRivalGen[] hormigas;
     ...
     * genético
     */
    public ColHFerDistRivalGen(Juego juego) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaFerDistRivalGen[113];
        nCiclos = 1;
        HormigaFerDistRivalGen.alfa = 0.19;
        HormigaFerDistRivalGen.beta = 0.2;
        HormigaFerDistRivalGen.ro = 0.15;
        HormigaFerDistRivalGen.restaPorDistancia = 0.21;
        HormigaFerDistRivalGen.pesoCoste = 0.8;
        HormigaFerDistRivalGen.pesoDistancia = 22.3;
        HormigaFerDistRivalGen.pesoRival = 20.6;
        minFeromona = 1;
        grafo = new Grafo(padre.getTablero());
        evGenetico = false;
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaFerDistRivalGen(grafo, padre.getActual(), new Posicion(5, -1));
            } else {
                hormigas[i] = new HormigaFerDistRivalGen(grafo, padre.getActual(), new Posicion(5, 17));
            }
        }
    }
     ...
     */
    public ColHFerDistRivalGen(Juego juego, int nHorm, int nCic, double alf,
            double bet, double r, double restaPorDist, double pCoste, double pDist,
            double minFer, double pRival, int elem, boolean inter) throws CloneNotSupportedException {
        evGenetico = true;
        padre = juego;
        nHormigas = nHorm;
        nCiclos = nCic;
        HormigaFerDistRivalGen.alfa = alf;
        HormigaFerDistRivalGen.beta = bet;
        HormigaFerDistRivalGen.ro = r;
        HormigaFerDistRivalGen.restaPorDistancia = restaPorDist;
        HormigaFerDistRivalGen.pesoCoste = pCoste;
        HormigaFerDistRivalGen.pesoDistancia = pDist;
        HormigaFerDistRivalGen.pesoRival = pRival;
        minFeromona = minFer;
        elemento = elem;
        intermedia = inter;
        hormigas = new HormigaFerDistRivalGen[nHormigas];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaFerDistRivalGen(grafo, padre.getActual(), new Posicion(5, -1));
    ...
    public void mover(Juego juego) throws CloneNotSupportedException {
        ...
        if (evGenetico) {
            if (intermedia) {
                GenOptColHFerDistRival.nTurnosInter[elemento]++;
            } else {
                GenOptColHFerDistRival.nTurnos[elemento]++;
            }
        }
...
//        // Volviendo las hormigas nada más llegar a la meta rival
//        int[] ciclos = new int[hormigas.length];
//
//        for (int j = 0; j < hormigas.length; j++) {
//            hormigas[j].limpiaCamino();
//        }
...
//        // (para que en las siguientes iteraciones usen la
//        // feromona de las anteriores)
//        for (int n = 0; maxCiclos(ciclos) < nCiclos && !Thread.interrupted() && !finTurno; n++) {
//
//            if (n % 10 == 0) {
//                Hormiga.actualizaTablasDec(grafo);
//            }
//
//            for (int j = 0; j < hormigas.length; j++) {
//                // Se mueve a cada hormiga y si ha llegado se le hace volver y se limpia su camino
//                if (hormigas[j].mover() && hormigas[j].getCamino().size() > 0) {
//                    // Ha llegado la hormiga, vuelve
//                    hormigas[j].volver();
//                    hormigas[j].limpiaCamino();
//                    ciclos[j]++;
//                }
...
//
//
//            // Se disipa la feromona cada 20 movimientos
//            if (n % 50 == 0) {
//                Hormiga.disiparFeromona(grafo);
//            }
//        }
        ...
        while (!fin && !Thread.interrupted() && !finTurno) {
            ...
            if (!primerMov && enlaces[enlace].getFeromona() < minFeromona) {
                if (evGenetico) {
                    ...
                    if (intermedia) {
                        GenOptColHFerDistRival.tiempoTotalInter[elemento] = t2 - t1;
                    } else {
                        GenOptColHFerDistRival.tiempoTotal[elemento] = t2 - t1;
                    }
                }
                return;
            }
    ...
    /**
     * Método que devuelve el número máximo de un array
     * @param ciclos Array con los ciclos de cada hormiga
     * @return Número mayor de ciclos realizados
     */
    public int maxCiclos(int[] ciclos) {
        int max = ciclos[0];
