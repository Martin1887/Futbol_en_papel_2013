...
/**
 * Controlador de la colonia de hormigas que usan los costes mínimos en lugar
 * de los reales
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class ColoniaHCostesMin extends Controlador {

    /**
     * Hormigas
     */
    private HormigaCostesMin[] hormigas;
     ...
     */
    public ColoniaHCostesMin(Juego juego) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaCostesMin[87];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaCostesMin(grafo, padre.getActual(), new Posicion(5, -1));
            } else {
                hormigas[i] = new HormigaCostesMin(grafo, padre.getActual(), new Posicion(5, 17));
            }
        }
    }
     ...
     */
    public ColoniaHCostesMin(Juego juego, int nHormigas) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new HormigaCostesMin[nHormigas];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new HormigaCostesMin(grafo, padre.getActual(), new Posicion(5, -1));
