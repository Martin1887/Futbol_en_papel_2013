...
/**
 * Controlador de la colonia de hormigas
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class ColoniaH extends Controlador {
     ...
     */
    public ColoniaH(Juego juego) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new Hormiga[87];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new Hormiga(grafo, padre.getActual(), new Posicion(5, -1));
            } else {
                hormigas[i] = new Hormiga(grafo, padre.getActual(), new Posicion(5, 17));
            }
        }
    }
     ...
     */
    public ColoniaH(Juego juego, int nHormigas) throws CloneNotSupportedException {
        padre = juego;
        hormigas = new Hormiga[nHormigas];
        grafo = new Grafo(padre.getTablero());
        for (int i = 0; i < hormigas.length; i++) {
            if (padre.getTurnoJug1()) {
                hormigas[i] = new Hormiga(grafo, padre.getActual(), new Posicion(5, -1));
