 ...
 * minimax con poda alfa-beta con costes
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class AlfaBetaConCostes extends Controlador {
    ...
    public void mover(Juego juego) throws Exception {
        ...
        if (padre.getTurnoJug1()) {
            ...
            // Se le suma 1 al Y porque el tablero de estados incluye la portería en y = 0
            raiz = tableroEstados[padre.getActual().getX()][padre.getActual().getY() + 1];
        } else {
        ...
        // Se le llamaría con el índice que corresponde al último movimiento hacia
        // la posición actual, pero como no se sabe se hace con el primero que
        // tiene un enlace marcado, ya que desde un enlace marcado nunca se va a
        // dar valor al estado
        int indice = 0;
        Nodo actual = padre.getTablero().getNodos()[padre.getActual().getX()]
                [padre.getActual().getY()];
        int enlaceDesdeActual = 0;
        ...
        double v = minValor(raiz, -INFINITO, INFINITO, 0, indice, 0, new LinkedList<EstadoEstatico>());
         ...
         * imposibles)
         */
        for (int j = 0; !fin &&!finTurno && !Thread.interrupted(); j++) {
            ...
            if (j == 0) {   // si es el 1er mov. se va al que dice el alfa-beta
                ...
                for (int n = 0; n < raiz.getHijos().length; n++) {
                    if (raiz.getHijos()[n] != null
                            && raiz.getHijos()[n].getValor(n) < min
                            // No se coge si vale -INFINITO porque ese valor lo
                            // ha puesto MAX por otro lado distinto
                            && raiz.getHijos()[n].getValor(n) != -INFINITO
                            // No se coge como mínimo si el enlace está marcado
                            && !padre.getTablero().getNodos()[raiz.getPosicion().getX()]
                            [raiz.getPosicion().getY()].getEnlaces()[enlaceAHijo(n)].getMarcado()) {
                        min = raiz.getHijos()[n].getValor(n);
                    }
                }
                ...
                for (int i = 0; i < raiz.getHijos().length && !finInterno; i++) {                   
                    ...
                    if (min == INFINITO) {
                        ...
                        // marcado
                        if (raiz.getHijos()[i] != null
                                && raiz.getHijos()[i].getValor(i) == min
                                && !padre.getTablero().getNodos()[raiz.getPosicion().getX()]
     ...
     * @param alfa
     * @param beta
     * @param coste El coste el enlace que conecta este estado con el anterior
     * @param indice El índice del hijo al que corresponde
     * @param profRebotes Indica el número de veces que se ha llamado al mismo método consecutivamente
     * @param anteriores Lista con los estados anteriores de la recursión
     * @return El valor del mejor hijo
     */
    public double maxValor(EstadoEstatico estado, double alfa, double beta,
            double coste, int indice, int profRebotes, LinkedList<EstadoEstatico> anteriores) {
        double v;
        

        anteriores.add(estado);

        if (testCorte(estado, profundidad)) {
            double ev = fEvaluacion(estado, MAX_VALOR) + coste;
            estado.setValor(indice, ev);
            anteriores.remove(estado);
            return ev;
        }
        ...
        for (int i = estado.getHijos().length - 1; i >= 0; i--) {
            int enlace = enlaceAHijo(i);
            // Antes de nada se comprueba que el hijo exista y si es un estado
            // de la portería
            if (estado.getHijos()[i] != null) {
                if ((estado.getHijos()[i].getPosicion().getX() == 5
                    || estado.getHijos()[i].getPosicion().getX() == 4
                    || estado.getHijos()[i].getPosicion().getX() == 6) && (padre.getTurnoJug1()
                    && estado.getHijos()[i].getPosicion().getY() == -1 || !padre.getTurnoJug1()
                    && estado.getHijos()[i].getPosicion().getY() == 17)) {
                    v = Math.max(0, v);
                    estado.getHijos()[i].setValor(i, 0);
                } else if ((estado.getHijos()[i].getPosicion().getX() == 5
                    || estado.getHijos()[i].getPosicion().getX() == 4
                    || estado.getHijos()[i].getPosicion().getX() == 6) && (padre.getTurnoJug1()
                    && estado.getHijos()[i].getPosicion().getY() == 17 || !padre.getTurnoJug1()
                    && estado.getHijos()[i].getPosicion().getY() == -1)) {
                    v = Math.max(INFINITO, v);
                    estado.getHijos()[i].setValor(i, INFINITO);
                } else if (estado.getHijos()[i].getPosicion().getX() >= 0
                        && estado.getHijos()[i].getPosicion().getX() < 11
                        && estado.getHijos()[i].getPosicion().getY() >= 0
                        && estado.getHijos()[i].getPosicion().getX() < 17) {
                    if (grafo.getGrafo().getNodos()[estado.getPosicion().getX()]
                        ...
                        v = Math.max(-INFINITO, v);
                        estado.getHijos()[i].setValor(i, -INFINITO);
                    } else if (anteriores.contains(estado.getHijos()[i])) {
                        // Si el estado ya ha sido visitado la evaluación a partir
                        // de aquí va a ser necesariamente peor, pues no hay costes
                        // nulos ni negativos, por lo que se pone a -INFINITO
                        v = Math.max(-INFINITO, v);
                        estado.getHijos()[i].setValor(i, -INFINITO);
                    } else if (enlacesInaccesibles7(estado.getHijos()[i].getPosicion())) {
                        v = Math.max(-INFINITO, v);
                        estado.getHijos()[i].setValor(i, -INFINITO);
                    } else if (grafo.getGrafo().getNodos()[estado.getHijos()[i]
                            .getPosicion().getX()][estado.getHijos()[i]
                            .getPosicion().getY()] != null
                            && grafo.getGrafo().getNodos()[estado.getHijos()[i]
                            .getPosicion().getX()][estado.getHijos()[i]
                            .getPosicion().getY()].getMarcado()
                            && profRebotes < 15) {
                        ...
                        // Se calculan de nuevo los costes con el enlace marcado
                        grafo.calcularCostes();
                        double aux = Math.max(v, maxValor(estado.getHijos()[i],
                                alfa, beta, grafo.getGrafo().getNodos()[estado.getPosicion().getX()]
                                [estado.getPosicion().getY()].getEnlaces()[enlace]
                                .getCoste(grafo.obtenerIndiceCoste
                                (estado.getPosicion(), estado.getHijos()[i]
                                .getPosicion())) + coste, i, profRebotes + 1, anteriores));
 
                        v = aux;

                        // No se marca definitivamente hasta el final del método
                        grafo.getGrafo().getNodos()[estado.getPosicion().getX()]
                        ...
                        double aux = Math.max(v, minValor(estado.getHijos()[i],
                                ...
                                .getCoste(grafo.obtenerIndiceCoste
                                (estado.getPosicion(), estado.getHijos()[i]
                                .getPosicion())) + coste, i, 0, anteriores));
     ...
     * @param alfa
     * @param beta
     * @param coste El coste del enlace que conecta este estado con el anterior
     * @param indice El índice del hijo al que corresponde
     * @param profRebotes Indica el número de veces que se ha llamado al mismo método consecutivamente
     * @param anteriores Lista con los estados anteriores de la recursión
     * @return El valor del peor hijo (del mejor hijo para el contrario)
     */
    public double minValor(EstadoEstatico estado, double alfa, double beta,
            double coste, int indice, int profRebotes, LinkedList<EstadoEstatico> anteriores) {
        double v;
        ...
        if (testCorte(estado, profundidad)) {
            double ev = fEvaluacion(estado, MIN_VALOR) + coste;
            estado.setValor(indice, ev);
            anteriores.remove(estado);
            return ev;
        }
        ...
        // Se calcula el valor máximo de los sucesores, teniendo en cuenta los
     ...
     */
    public boolean testCorte(EstadoEstatico estado, int profun) {
     ...
     */
    public double fEvaluacion(EstadoEstatico actual, int metodo) {
...
//            LinkedList<EstadoEstatico> estados = new LinkedList<EstadoEstatico>();
...
//            LinkedList<EstadoEstatico> antecesores = new LinkedList<EstadoEstatico>();
//            // Sucesores no antecesores de estado
//            EstadoEstatico[] sucesores = new EstadoEstatico[8];
...
//            }
     ...
     */
    public double distancia(EstadoEstatico estado) {
        double resultado = 0;
    ...
    /**
     * Método que recibe un vector de double (f) y las listas g y h y
    ...
    public static void quickSort(double[] vector, LinkedList<Double> g,
            LinkedList<Double> h, LinkedList<EstadoEstatico> estados, int izq, int der) {
        ...
        EstadoEstatico auxE; // variable auxiliar para intercambio de estados
    ...
    /**
     * Devuelve el índice del enlace que conecta el estado actual con el hijo
     * pasado por parámetro
     * @param hijo El índice del hijo
     * @return El índice del enlace que conecta el estado actual con el hijo
     * pasado por parámetro
     */
        ...
        if (padre.getTurnoJug1()) {
            ...
            for (int i = 1; i < 10; i++) {
                if (i == 4) {
                    ...
                    hijos = new EstadoEstaticoJ2[8];
                    hijos[5] = (EstadoEstaticoJ2) tableroEstados[i][16];
                    hijos[6] = (EstadoEstaticoJ2) tableroEstados[i - 1][16];
                    hijos[7] = (EstadoEstaticoJ2) tableroEstados[i + 1][16];
                    hijos[3] = (EstadoEstaticoJ2) tableroEstados[i - 1][17];
                    hijos[4] = (EstadoEstaticoJ2) tableroEstados[i + 1][17];
                    hijos[0] = null;
                    hijos[1] = (EstadoEstaticoJ2) tableroEstados[i - 1][18];
