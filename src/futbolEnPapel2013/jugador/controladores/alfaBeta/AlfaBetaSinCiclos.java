 ...
 * minimax con poda alfa-beta evitando que en la recursión vuelva a estados
 * anteriores en el mismo método (es decir, rebotando) dando a estos valor
 * INFINITO si es minValor y -INFINITO si es maxValor
 * @author Marcos Martín Pozo Delgado
 * @version 1.0
 */
public class AlfaBetaSinCiclos extends Controlador {
    ...
    public void mover(Juego juego) throws Exception {
        ...
        for (int i = 0; i < actual.getEnlaces().length; i++) {
            if (actual.getEnlaces()[i] != null && actual.getEnlaces()[i].getMarcado()) {
                enlaceDesdeActual = i;
            }
        }
        if (padre.getTurnoJug1()) {
        ...
        double v = minValor(raiz, -INFINITO, INFINITO, indice, new LinkedList<EstadoEstatico>());
     ...
     * @param anteriores Lista con los estados anteriores en la recursión
     * @return El valor del mejor hijo
     */
    public double maxValor(EstadoEstatico estado, double alfa, double beta, int indice,
            LinkedList<EstadoEstatico> anteriores) {
        double v;
            ...
            if (estado.getHijos()[i] != null) {
                if ((estado.getHijos()[i].getPosicion().getX() == 5
                    ...
                    if (grafo.getGrafo().getNodos()[estado.getPosicion().getX()]
                        ...
                        // Si el estado ya ha sido visitado se le pone valor
                        // INFINITO para que no haya ciclos aumentando demasiado
                        ...
                        double aux = Math.max(v, minValor(estado.getHijos()[i],
                                alfa, beta, i, new LinkedList<EstadoEstatico>()));
     ...
     */
    public double minValor(EstadoEstatico estado, double alfa, double beta, int indice,
            LinkedList<EstadoEstatico> anteriores) {
        double v;
